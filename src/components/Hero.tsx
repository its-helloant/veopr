    ...
    <section className="hero-gradient py-12 sm:py-16 md:py-24 text-white relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto padding-mobile grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 items-center relative z-10">
        ...
        <div className="flex justify-center lg:justify-end">
          <Card className="w-full max-w-md shadow-2xl group cursor-pointer transition-all duration-300 hover:shadow-3xl focus-within:shadow-3xl">
            <CardBody className="p-0 relative overflow-hidden">
              <img 
                src="/dia-a-dia-logo.png" 
                alt="Programa destacado Día a Día - Ver episodio" 
                className="w-full h-auto transition-transform duration-300 group-hover:scale-105 group-focus-within:scale-105"
              />
              <div className="absolute inset-0 bg-black bg-opacity-40 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                <Button
                  isIconOnly
                  size="lg"
                  onClick={handleVideoPlay}
                  className="bg-white bg-opacity-90 text-black text-xl sm:text-2xl hover:scale-110 focus:scale-110 transition-transform focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-black touch-target"
                  radius="full"
                  aria-label="Reproducir programa destacado"
                >
                  <span aria-hidden="true">▶</span>
                </Button>
              </div>
              
              {/* Hidden button for keyboard navigation */}
              <button
                onClick={handleVideoPlay}
                className="absolute inset-0 w-full h-full bg-transparent border-none outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-blue-600 focus:bg-black focus:bg-opacity-20 transition-all"
